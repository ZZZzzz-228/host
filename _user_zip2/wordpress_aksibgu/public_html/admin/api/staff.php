 ...
 *
 * Маппинг JS ↔ БД:
 *   JS: position_title  ↔  DB: position
 *   JS: office_hours    ↔  DB: schedule
 *   JS: is_published    ↔  DB: is_active
 *   JS: department      ↔  DB: role        (college / career_center / other)
 *   JS: color_hex       ↔  DB: color_hex   (колонка добавляется через alter_staff.sql)
 */
...
/* ── Вспомогательная: собрать поля из тела запроса ─────────────────────── */
function staffFields(array $d): array
{
    return [
        ...
        'email'      => trim($d['email']           ?? ''),
        'phone'      => trim($d['phone']           ?? ''),
        // JS поле "department" (college/career_center/other) хранится в колонке role
...
/* ── Нормализовать строку из БД для ответа (добавить алиасы для JS) ──────── */
function staffRow(array $row): array
{
    ...
    // JS ожидает поле department, а в БД хранится role
...
if ($method === 'GET') {
    ...
    // JS фильтр по department → в БД это колонка role
