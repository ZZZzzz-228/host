...
function specFields(array $d): array {
    return [
        ...
        // ── НОВЫЕ ПОЛЯ ──
        'base_9'             => isset($d['base_9'])       ? (int)(bool)$d['base_9']       : 0,
        'base_11'            => isset($d['base_11'])      ? (int)(bool)$d['base_11']      : 0,
        'has_budget'         => isset($d['has_budget'])   ? (int)(bool)$d['has_budget']   : 0,
        'budget_seats'       => isset($d['budget_seats']) ? max(0, (int)$d['budget_seats']) : 0,
        // ──────────────────
...
if ($method === 'PATCH') {
    ...
    if (!$id) { http_response_code(400); json(['error' => 'id required']); }
    $d = jsonBody();
    $allowed = ['is_published', 'sort_order', 'color_hex', 'base_9', 'base_11', 'has_budget', 'budget_seats'];
    $set = []; $params = [':id' => $id];
