...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   Новые поля: base_9, base_11, has_budget, budget_seats
   ══════════════════════════════════════════════════════════════════════════ */
...
function openSpec(it) {
    ...
    // ── НОВЫЕ ПОЛЯ ──
    $('spBase9').checked     = it ? !!+it.base_9 : false;
    $('spBase11').checked    = it ? !!+it.base_11 : false;
    $('spHasBudget').checked = it ? !!+it.has_budget : false;
    $('spBudgetSeats').value = it ? (it.budget_seats || 0) : 0;
    toggleBudgetSeatsRow();
    $('mSpecT').textContent = it ? 'Редактировать специальность' : 'Добавить специальность';
    om('mSpec');
}

/** Показывает/скрывает поле "Количество бюджетных мест" в зависимости от чекбокса has_budget */
function toggleBudgetSeatsRow() {
    const has = $('spHasBudget').checked;
    const row = $('spBudgetSeatsRow');
...
async function saveSpec() {
    const id = $('spId').value;
    const hasBudget = $('spHasBudget').checked;
    const d = {
        ...
        // ── НОВЫЕ ПОЛЯ ──
        base_9:        $('spBase9').checked  ? 1 : 0,
        base_11:       $('spBase11').checked ? 1 : 0,
        has_budget:    hasBudget ? 1 : 0,
        budget_seats:  hasBudget ? (+$('spBudgetSeats').value || 0) : 0
    };
...
/* ══════════════════════════════════════════════════════════════════════════
   DEPARTMENTS / GROUPS / DISCIPLINES / SCHEDULE (без изменений)
   ══════════════════════════════════════════════════════════════════════════ */
...
function openDept(it) {
    $('dpId').value=it?it.id:'';$('dpNm').value=it?it.name:'';$('dpCd').value=it?(it.code||''):'';
    $('dpHd').value=it?(it.head_name||''):'';$('dpPh').value=it?(it.phone||''):'';
    $('dpEm').value=it?(it.email||''):'';$('dpRm').value=it?(it.room||''):'';
    $('dpOrd').value=it?(it.sort_order||0):0;$('dpAct').checked=!it||!!+it.is_active;
    $('mDeptT').textContent=it?'Редактировать отделение':'Добавить отделение';om('mDept');
}
async function editDept(id){const r=await api('GET',`api/departments.php?id=${id}`);if(r&&r.id)openDept(r);else if(r&&r.data)openDept(r.data);}
async function saveDept(){
    const id=$('dpId').value;
    const d={name:v('dpNm'),code:v('dpCd'),head_name:v('dpHd'),phone:v('dpPh'),email:v('dpEm'),room:v('dpRm'),sort_order:+$('dpOrd').value,is_active:$('dpAct').checked?1:0};
...
async function delDept(id){if(!confirm('Удалить отделение?'))return;await api('DELETE',`api/departments.php?id=${id}`);toast('Удалено');loadDepts();}

async function loadGroups() {
    const r = await api('GET', 'api/groups.php?limit=100');
    const d = r && r.data ? r.data : [];
    $('grTb').innerHTML = d.length ? d.map(g => `<tr>
        <td>${g.id}</td><td><strong>${esc(g.name)}</strong></td><td>${g.course||'—'}</td>
        <td>${esc(g.specialty_title||'—')}</td><td>${esc(g.study_year||'—')}</td>
...
function openGroup(it){$('grId').value=it?it.id:'';$('grNm').value=it?it.name:'';$('grCr').value=it?(it.course||''):'';$('grYr').value=it?(it.study_year||''):'';$('grOrd').value=it?(it.sort_order||0):0;$('grAct').checked=!it||!!+it.is_active;$('mGroupT').textContent=it?'Редактировать группу':'Добавить группу';om('mGroup');}
async function editGroup(id){const r=await api('GET',`api/groups.php?id=${id}`);if(r&&r.id)openGroup(r);else if(r&&r.data)openGroup(r.data);}
async function saveGroup(){const id=$('grId').value;const d={name:v('grNm'),course:+$('grCr').value||null,study_year:v('grYr'),sort_order:+$('grOrd').value,is_active:$('grAct').checked?1:0};if(!d.name){toast('Введите название','error');return;}const r=await api(id?'PUT':'POST',id?`api/groups.php?id=${id}`:'api/groups.php',d);if(r&&(r.id||r.success)){toast(id?'Обновлено':'Создано');cm('mGroup');loadGroups();}else toast((r&&(r.error||r.message))||'Ошибка','error');}
async function delGroup(id){if(!confirm('Удалить группу?'))return;await api('DELETE',`api/groups.php?id=${id}`);toast('Удалено');loadGroups();}

async function loadDiscs() {
    const r = await api('GET', 'api/disciplines.php?limit=100');
    const d = r && r.data ? r.data : [];
    $('dcTb').innerHTML = d.length ? d.map(dc => `<tr>
        <td>${dc.id}</td><td><strong>${esc(dc.name)}</strong></td>
        <td>${esc(dc.code || '—')}</td>
...
function openDisc(it){$('dcId').value=it?it.id:'';$('dcNm').value=it?it.name:'';$('dcCd').value=it?(it.code||''):'';$('dcDesc').value=it?(it.description||''):'';$('dcAct').checked=!it||!!+it.is_active;$('mDiscT').textContent=it?'Редактировать дисциплину':'Добавить дисциплину';om('mDisc');}
async function editDisc(id){const r=await api('GET',`api/disciplines.php?id=${id}`);if(r&&r.id)openDisc(r);else if(r&&r.data)openDisc(r.data);}
async function saveDisc(){const id=$('dcId').value;const d={name:v('dcNm'),code:v('dcCd'),description:v('dcDesc'),is_active:$('dcAct').checked?1:0};if(!d.name){toast('Введите название','error');return;}const r=await api(id?'PUT':'POST',id?`api/disciplines.php?id=${id}`:'api/disciplines.php',d);if(r&&(r.id||r.success)){toast(id?'Обновлено':'Создано');cm('mDisc');loadDiscs();}else toast((r&&(r.error||r.message))||'Ошибка','error');}
async function delDisc(id){if(!confirm('Удалить дисциплину?'))return;await api('DELETE',`api/disciplines.php?id=${id}`);toast('Удалено');loadDiscs();}
...
async function loadSched() {
    ...
    $('scTb').innerHTML = d.length ? d.map(s => `<tr>
        <td>${DAYS[s.day_of_week] || s.day_of_week}</td><td>${s.lesson_number}</td>
        <td>${s.time_start ? s.time_start + ' – ' + s.time_end : '—'}&nbsp;${bdg(wRu[s.week_type] || '', 'bm')}</td>
        <td><strong>${esc(s.group_name)}</strong></td><td>${esc(s.subject)}</td>
        <td>${esc(s.teacher || '—')}</td><td>${esc(s.room || '—')}</td>
        <td><div class="tbl-act">
...
function openSched(it){$('scId').value=it?it.id:'';$('scGr').value=it?it.group_name:'';$('scDay').value=it?it.day_of_week:1;$('scLes').value=it?it.lesson_number:1;$('scTS').value=it?(it.time_start||''):'';$('scTE').value=it?(it.time_end||''):'';$('scWk').value=it?(it.week_type||'all'):'all';$('scSbj').value=it?it.subject:'';$('scTch').value=it?(it.teacher||''):'';$('scRm').value=it?(it.room||''):'';$('mSchedT').textContent=it?'Редактировать занятие':'Добавить занятие';om('mSched');}
async function editSched(id){const r=await api('GET',`api/schedule.php?id=${id}`);if(r&&r.id)openSched(r);else if(r&&r.data)openSched(r.data);}
async function saveSched(){const id=$('scId').value;const d={group_name:v('scGr'),day_of_week:+$('scDay').value,lesson_number:+$('scLes').value,time_start:v('scTS')||null,time_end:v('scTE')||null,week_type:$('scWk').value,subject:v('scSbj'),teacher:v('scTch'),room:v('scRm')};if(!d.group_name||!d.subject){toast('Заполните Группу и Предмет','error');return;}const r=await api(id?'PUT':'POST',id?`api/schedule.php?id=${id}`:'api/schedule.php',d);if(r&&(r.id||r.success)){toast(id?'Обновлено':'Создано');cm('mSched');loadSched();}else toast((r&&(r.error||r.message))||'Ошибка','error');}
async function delSched(id){if(!confirm('Удалить занятие?'))return;await api('DELETE',`api/schedule.php?id=${id}`);toast('Удалено');loadSched();}

/* ══════════════════════════════════════════════════════════════════════════
   STAFF (сотрудники) — теперь с DRAG & DROP сортировкой
   ══════════════════════════════════════════════════════════════════════════ */
let _staffSortable = null; // экземпляр Sortable
let _staffCurrent  = [];   // последний загруженный список (для drag&drop)

async function loadStaff() {
    const q = v('stfSrch'), dep = $('stfDep').value;
    let u = 'api/staff.php?limit=200';
    if (q) u += '&q=' + encodeURIComponent(q);
    if (dep) u += '&department=' + dep;
    const r = await api('GET', u);
    const d = r && r.data ? r.data : [];
    _staffCurrent = d.slice();
    const dRu = {college:'Колледж', career_center:'Карьерный центр', other:'Другое'};

    $('sfTb').innerHTML = d.length ? d.map(s => `<tr data-id="${s.id}">
        <td class="drag-handle" title="Перетащите, чтобы изменить порядок"><i class="fas fa-grip-vertical"></i></td>
        <td>${s.id}</td>
        <td>${s.photo_url
        ...
        <td>${pubBdg(s.is_published)}</td>
        <td><span class="sort-badge" style="display:inline-block;min-width:24px;text-align:center;padding:2px 8px;border-radius:8px;background:rgba(21,101,192,.1);color:#1565C0;font-weight:600;font-size:12px">${s.sort_order ?? 0}</span></td>
        <td><div class="tbl-act">
        ...
        </div></td></tr>`).join('') : `<tr><td colspan="9">${empty('Сотрудников нет')}</td></tr>`;

    // Инициализируем Sortable один раз; при повторных вызовах достаточно того, что DOM tbody тот же.
    initStaffSortable();
}

function initStaffSortable() {
    const tbody = document.getElementById('sfTb');
    if (!tbody) return;
    if (typeof Sortable === 'undefined') {
        console.warn('[staff] SortableJS not loaded — drag&drop disabled');
        return;
    }
    // Если уже инициализировано — не пересоздаём
    if (_staffSortable && _staffSortable.el === tbody) return;
    if (_staffSortable) { try { _staffSortable.destroy(); } catch(e){} _staffSortable = null; }

    _staffSortable = Sortable.create(tbody, {
        handle: '.drag-handle',
        animation: 180,
        ...
        onEnd: async () => {
            ...
            // Сразу обновляем бейджи порядка в UI (оптимистично)
            rows.forEach((tr, idx) => {
                const badge = tr.querySelector('.sort-badge');
                if (badge) badge.textContent = idx;
            });
            // Отправляем на сервер
            const r = await api('PATCH', 'api/staff.php?reorder=1', { items });
            if (r && r.success) {
                toast('Порядок обновлён');
            } else {
                toast((r && (r.error || r.message)) || 'Не удалось сохранить порядок', 'error');
                // В случае ошибки — перезагружаем список с сервера
                loadStaff();
            }
        }
    });
