...
if ($method === 'GET' && $uri === '/specialties') {
    ...
    $rows = $pdo->query(
        "SELECT id, code, title, short_title, description, duration_label, study_form_label,
                ...
                base_9, base_11, has_budget, budget_seats
         FROM specialties
         WHERE is_published = 1
    ...
    foreach ($rows as &$r) {
        $r['image_url']    = fix_url($r['image_url'] ?? '');
        // Приводим тинюинты к bool/int, чтобы JSON был стабилен
        $r['base_9']       = (int) ($r['base_9'] ?? 0);
        $r['base_11']      = (int) ($r['base_11'] ?? 0);
        $r['has_budget']   = (int) ($r['has_budget'] ?? 0);
        $r['budget_seats'] = (int) ($r['budget_seats'] ?? 0);
    }
    unset($r);
    json_out(['data' => $rows]);
}
...
// GET /staff
// ────────────────────────────────────────────────────────────────────────────
// ИСПРАВЛЕНО: возвращаем реальный color_hex из БД (был '' AS color_hex)
// ИСПРАВЛЕНО: при фильтре по department ищем по полю role (как и было),
//             но теперь с реальным color_hex
// ────────────────────────────────────────────────────────────────────────────
if ($method === 'GET' && $uri === '/staff') {
    ...
    if ($department !== '') {
        $st = $pdo->prepare(
            "SELECT id, full_name, position AS position_title, email, phone,
                    schedule AS office_hours, photo_url, color_hex
             FROM staff_members
    ...
    foreach ($rows as &$r) {
        ...
        // Гарантируем что color_hex не null
        if (empty($r['color_hex'])) {
            $r['color_hex'] = '#1565C0';
        }
    }
    unset($r);
